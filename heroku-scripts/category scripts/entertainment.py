...
# @task:  Master File which controls all replies extracted from shortlisted entertainment tweets. Find statistics on this auxiliary data
# and uploads a csv files to S3

import entertainment_replies
import pandas as pd
import sentibank
...
print("entertainment: Retrieving File from S3")
...
news_file = news_bucket.Object(key='entertainment.csv')
...
for index, row in df.iterrows():
    ...
    if row['retrieved'] == False:
        ...
        replies = entertainment_replies.reply_thread_maker(conv_ids)
